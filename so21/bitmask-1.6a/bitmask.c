...
{
  bitmask_t *temp;
  temp = malloc(offsetof(bitmask_t,bits) + h*((w - 1)/BITW_LEN + 1)*sizeof(BITW));  
  if (!temp) 
    return 0;
  temp->w = w;
  temp->h = h;
...
{
  const BITW *a_entry,*a_end;
  const BITW *b_entry;
  const BITW *ap,*app,*bp;
...
{
  const BITW *a_entry,*a_end, *b_entry, *ap, *bp;
...
/* End of Donald W. Gillies bitcount code */
   ...
   /* This piece taken from Jorg Arndt's "Algorithms for Programmers" */
  ...
  else if (BITW_LEN == 64)
    {
      ...
      n +=  n>> 8;                                    
      n +=  n>>16;                                    
      n +=  ((unsigned __int64)n)>>32; //Eli added in accordance with http://msdn.microsoft.com/en-us/library/cx0bb1cy(VS.80).aspx
      return  n & 0xff;
    }
  else
    {
...
{
  BITW *a_entry,*a_end, *ap;
  const BITW *b_entry, *b_end, *bp;
  ...
  if (xoffset >= 0) 
  ...
  else  
    {
      ...
      if (yoffset >= 0)
	{
	  b_entry = b->bits + b->h*(xoffset/BITW_LEN) + yoffset;
	  b_end = b_entry + MIN(a->h,b->h - yoffset);
	  a_entry = a->bits;
	}
      else
	{
	  b_entry = b->bits + b->h*(xoffset/BITW_LEN);
